...
export function isCorrect(question: Question, answer: string): boolean {
    //prettier-ignore
    return question.expected.trim().toLowerCase() === answer.trim().toLowerCase() ? true : false;
}
...
export function isValid(question: Question, answer: string): boolean {
    ...
    return true;
    //we don't need to check for other question types outside of the two defined
    //since questionType can only be 2, listed in question.ts
}
...
export function toMarkdown(question: Question): string {
    if (question.type === "multiple_choice_question") {
        const optionDisplay = question.options
            .map((option) => `- ${option}`)
            .join("\n");

        return `# ${question.name}\n${question.body}\n${optionDisplay}`;
    }
    return `# ${question.name}\n${question.body}`; //not a multi choice question
}
